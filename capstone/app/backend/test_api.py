...
auth_jwts = {
    'member': {"Authorization": "Bearer {}".format(os.environ['AUTH0_JWT_MEMBER'])},
    'admin': {"Authorization": "Bearer {}".format(os.environ['AUTH0_JWT_ADMIN'])}
}
...
class RacesTestCase(unittest.TestCase):
    ...
    def test_get_races(self):
        # success behavior
        result = self.client().get('/races') # no auth required
    ...
    def test_get_distances(self):
        ...
        result = self.client().get('distances/1', headers=auth_jwts['member'])
        data = result.get_json()
        self.assertEqual(result.status_code, 405)
        self.assertFalse(data['success'])
    ...
    def test_get_race_detail(self):
        ...
        result = self.client().get('/races-detail/1', headers=auth_jwts['member'])
        data = result.get_json()
        self.assertEqual(result.status_code, 200)
        self.assertTrue(data['success'])
        ...
        result = self.client().get('/races-detail/99999999', headers=auth_jwts['member'])
        data = result.get_json()
    ...
    def test_add_distance(self):
        ...
        result1 = self.client().get('/distances', headers=auth_jwts['member'])
        data1 = result1.get_json()
        ...
        result2 = self.client().get('distances', headers=auth_jwts['admin'])
        data2 = result2.get_json()
    ...
    def test_update_race(self):
        ...
        upd_site = self.client().patch('/races/{}'.format(race_id), 
            json={'website': new_site}, headers=auth_jwts['admin'])
        self.assertEqual(upd_site.status_code, 200)
        self.assertTrue(upd_site.get_json()['success'])
        # did we change it?
        changed_site = self.client().get('/races-detail/{}'.format(race_id),
            headers=auth_jwts['admin']).get_json()['race']['website']
        self.assertEqual(new_site, changed_site)
        # change it back
        upd_site = self.client().patch('/races/{}'.format(race_id), 
            json={'website': cur_site}, headers=auth_jwts['admin'])
        self.assertEqual(upd_site.status_code, 200)
        self.assertTrue(upd_site.get_json()['success'])
        # verify changed it back
        changed_back = self.client().get('/races-detail/{}'.format(race_id), 
            headers=auth_jwts['admin']).get_json()['race']['website']
    ...
    def test_update_distance(self):
        ...
        new_dist = self.client().patch('/distances/{}'.format(dist_id), 
                    json={'name': 'Ultramarathon','distance_km': dist_km}, headers=auth_jwts['admin'])
        ...
        dist_dict = self.client().get('/distances', headers=auth_jwts['admin']).get_json()['distances'][dist_id]
        ...
        no_dist = self.client().patch('/distances/1', json={'furlongs': 2}, headers=auth_jwts['admin'])
        self.assertEqual(no_dist.status_code, 422)
        self.assertFalse(no_dist.get_json()['success'])

        # check failure mode 2: no distance at ID
        no_dist = self.client().patch('/distances/99999999', json={}, headers=auth_jwts['admin'])
        self.assertEqual(no_dist.status_code, 422)
        self.assertFalse(no_dist.get_json()['success'])
    ...
    def test_delete_race(self):
        ...
        deleted = self.client().delete('/races/{}'.format(race_id), headers=auth_jwts['admin'])
        ...
        # check failure mode
        too_high = self.client().delete('/races/9999999999', headers=auth_jwts['admin'])
    ...
    def test_delete_distance(self):
        ...
        dist_id = str(max([int(id) for id in self.client().get('/distances', headers=auth_jwts['admin']).get_json()['distances'].keys()]))
        deleted = self.client().delete('/distances/{}'.format(dist_id), headers=auth_jwts['admin'])
        self.assertEqual(deleted.status_code, 200)
        self.assertTrue(deleted.get_json()['success'])

         # check to see if dist_id still is there
        self.assertNotIn(dist_id, self.client().get('/distances', headers=auth_jwts['admin']).get_json()['distances'].keys())
        
        # check failure mode
        too_high = self.client().delete('/distances/9999999999', headers=auth_jwts['admin'])
        self.assertEqual(too_high.status_code, 422)
        self.assertFalse(too_high.get_json()['success'])
